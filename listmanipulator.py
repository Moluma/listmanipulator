...
def add_sdate(base_list):
    my_file = open(sys.argv[2]+"_asd" ,"w")
    templist = []
    day = []
    day.append(str(input("Set day with two digits (if unknown just hit enter): ")))
    if day == ['']:
        day = days
    month = []
    month.append(str(input("Set month with two digits (if unknown just hit enter): ")))
    if month == ['']:
        month = months
    year = []
    year.append(str(input("Set year with two digits (if unknown just hit enter): ")))
    if year == ['']:
        year = years
    for line in base_list:
        templist.append(str(line).replace('\n', ""))
    combinations = (it.product(templist, day, month, year, repeat=1))
    for i in combinations:
        guess = ""
        for e in i:
            guess = str(guess) + str(e)
